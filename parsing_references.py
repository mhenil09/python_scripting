...
def fetch_references():
    ...
    for res in result:
        ...
        sectionNo = link.split("/")[length - 1]
        file = do_file_operations(sectionNo + "_" + today, link)
        bs_data = fetch_xml_content(file)
        for div in bs_data.find_all("div", {"class": "section"}):
            main_text = None
...
def do_file_operations(file, link):
    file_name = "xml_files/references/" + file + ".xml"
    f = open(file_name, "w")
